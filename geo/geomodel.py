...
class GeoModel(db.Model):
  ...
  def bounding_box_fetch(cls, query, bbox, max_results=1000,
    ...
    Raises:
      Any exceptions that .fetch() can raise.
    """
    ...
    if query_geocells:
      if query._Query__orderings:
        ...
        util.merge_in_place(cmp_fn=ordering_fn, *cell_results)
  ...
  def proximity_fetch(cls, query, center, max_results=10, max_distance=0):
    ...
    def _merge_results_in_place(a, b):
      util.merge_in_place(a, b, cmp_fn=lambda x, y: cmp(x[1], y[1]),
